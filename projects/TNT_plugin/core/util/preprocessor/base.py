...
class Preprocessor(Dataset):
    ...
    """
    def __init__(self, root_dir, algo="tnt", obs_horizon=50, obs_range=1500, pred_horizon=50):
    ...
    def process_and_save_tl(self, dataframe: pd.DataFrame, tl_df, seq_id, dir_=None, map_feat=True):
        df_processed = self.process(dataframe, tl_df, seq_id, map_feat)
        self.save(df_processed, seq_id, dir_)

        return []

    def process_and_save_road(self, dataframe: pd.DataFrame, road_df, seq_id, dir_=None, map_feat=True):
        df_processed = self.process(dataframe, road_df, seq_id, map_feat)
        self.save(df_processed, seq_id, dir_)
