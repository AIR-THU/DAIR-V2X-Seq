    ...
    def test(self,
        ...
        if plot:
            ...
            for key in forecasted_trajectories.keys():
                # ax.set_xlim(-15, 15)
                ax.set_xlim(gt_trajectories[key][0][0]-15, gt_trajectories[key][0][0]+15)
                ax.set_ylim(gt_trajectories[key][0][1]-15, gt_trajectories[key][0][1]+15)
                show_pred_and_gt(ax, gt_trajectories[key], forecasted_trajectories[key])
                plt.pause(3)
                # plt.pause(3)
                plt.savefig(self.save_folder + '/' + str(key) + '.png')
                ax.clear()
