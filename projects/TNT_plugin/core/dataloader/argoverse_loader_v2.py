...
class ArgoverseInMem(InMemoryDataset):
    ...
    def process(self):
        ...
        for raw_path in tqdm(self.raw_paths, desc="Loading Raw Data..."):
            ...
            road_traj_num = 0
            if 'road_feats' in raw_data.keys():
                road_traj_num += raw_data['road_feats'].values[0].shape[0]
            traj_lens.append(traj_num + road_traj_num)

            lane_num = raw_data['graph'].values[0]['lane_idcs'].max() + 1
            valid_lens.append(traj_num + road_traj_num + lane_num)
        ...
        print("\n[TFD]: The maximum of valid length is {}.".format(num_valid_len_max))
        print("[TFD]: The maximum of no. of candidates is {}.".format(num_candidate_max))
        ...
        for ind, raw_path in enumerate(tqdm(self.raw_paths, desc="Transforming the data to GraphData...")):
            ...
            # input data
            '''
            if raw_data['road_feats'].values[0].shape[0] == 0:
                print(raw_path)
                continue
            '''
    ...
    def _get_x(data_seq):
        ...
        '''
        print('--test--')
        print(data_seq['feats'].values[0].shape)
        print(data_seq['road_feats'].values[0].shape)
        print(np.concatenate((data_seq['feats'].values[0], data_seq['road_feats'].values[0]), axis=0).shape)
        '''
        if 'road_feats' in data_seq.keys() and data_seq['road_feats'].values[0].shape[0]:
            traj_feats = np.concatenate((data_seq['feats'].values[0], data_seq['road_feats'].values[0]), axis=0)
            traj_has_obss = np.concatenate((data_seq['has_obss'].values[0], data_seq['road_has_obss'].values[0]), axis=0)
        else:
